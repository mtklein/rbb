...
static void load_565_eval_f(struct cfg const *cfg, v8f reg[]) {
    struct load const *node = (struct load const*)cfg;
    v8s px;
...
struct cfg* load_565(uint16_t const *src) {
    struct load *node = malloc(sizeof *node);
    node->cfg = (struct cfg) {
        .free   = load_free,
...
static void load_fp16_eval_f(struct cfg const *cfg, v8f reg[]) {
    struct load const *node = (struct load const*)cfg;
    struct rgba_fp16 const *src = node->src;
    for (int i = 0; i < 8; i++) {
...
struct cfg* store_565(uint16_t *dst, struct cfg *rgb) {
    struct store *node = malloc(sizeof *node);
    node->cfg = (struct cfg) {
        .free   = store_free,
